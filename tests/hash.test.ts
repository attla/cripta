...
describe('Hash', () => {
  it.each(Object.keys(dataset) as (keyof typeof dataset)[])('%s', async (alg) => {
    expect(bytesToHex(await hash[alg](plain))).toBe(dataset[alg])
  })

  describe('Hash generation', () => {
    it.each(strings)('%s', async (_, value) => {
      // Test without salt
      const hash1 = await hash.hash(value)
      expect(hash1).toBeString()
      expect(hash1.length).toBeGreaterThanOrEqual(hash.HASH_LEN)

      // Test with salt
      const salt = 'testsalt'
      const hash2 = await hash.hash(value, salt)
    ...
    it('should return empty string for invalid input', async () => {
      expect(await hash.hash('')).toBe('')
      expect(await hash.hash(null as any)).toBe('')
      expect(await hash.hash(undefined as any)).toBe('')
      expect(await hash.hash(123 as any)).toBe('')
    })
  })

  describe('Hash comparison', () => {
    it.each(strings)('%s', async (_, value) => {
      const val = await hash.hash(value)

      expect(await hash.compare(value, val)).toBeTrue()
      expect(await hash.compare(value + 'x', val)).toBeFalse()
      expect(await hash.compare(value, val + 'x')).toBeFalse()
    })

    it('should return false for invalid input', async () => {
      expect(await hash.compare('', 'hash')).toBeFalse()
      expect(await hash.compare('value', '')).toBeFalse()
      expect(await hash.compare(null as any, 'hash')).toBeFalse()
      expect(await hash.compare('value', null as any)).toBeFalse()
    })
  })
  ...
  describe('Salt extraction', () => {
    it.each(strings)('%s', async (_, value) => {
      const salt = 'testsalt'
      const val = await hash.hash(value, salt)
